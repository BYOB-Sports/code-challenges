...
import { filterCourts, getAllCourts, mockCourts, searchCourts } from './index';
...
const surfaceStats = mockCourts.reduce(
  (acc, court) => {
...
const priceRanges = {
  ...
  midRange: mockCourts.filter(c => c.pricePerHour >= 30 && c.pricePerHour < 60)
    .length,
...
const avgRating =
  mockCourts.reduce((sum, court) => sum + court.rating, 0) / mockCourts.length;
